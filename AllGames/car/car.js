...
// avoid error if there is no "startBtn" on the page
...
// Use Matter module names with capital M
...
let playerScore = 0;
let computerScore = 0;
let gameOver = false; // Game khatam hua ya nahi
let winner = ""; // Kaun jeeta uska message
...
function draw() {
  if (gameOver) {
    ...
    rectMode(CENTER);
    fill(255);
    rect(width / 2, height / 2 + 40, 200, 60, 15);
  ...
  if (!gameStarted) {
    ...
    fill(255);
    textSize(36);
    text("CAR SOCCER", width / 2, height / 2 - 120);

    // Button (just visual, mousePressed handles click)
    fill(255);
  ...
  if (ball.didScore()) {
    ...
    shootSparks(x, y);
    Matter.Body.setPosition(ball.body, { x: width / 2, y: height / 2 });
  ...
  if (goalWaitPeriod) {
    ...
    textSize(64);
    text("IT'S A GOAL!", width / 2 - 200, height / 2);
  }
...
function addWalls() {
  ...
  const bottomWall = new Wall(width / 2, height + wt2, width, wallThickness, 0);
  const topWall = new Wall(width / 2, -wt2, width, wallThickness, 0);

  const leftWall = new Wall(-wt2, height / 2, height, wallThickness, PI / 2);
  const rightWall = new Wall(width + wt2, height / 2, height, wallThickness, PI / 2);
}
